	...
	<view class="category-box">
		...
		<view class="search">
			<uv-input placeholder="前置图标" prefixIcon="search" prefixIconStyle="font-size: 22px;color: #909399"
				style="height: 50rpx;"></uv-input>
		</view>
		<!-- 菜单 -->
		<view class="menu">
			<scroll-view class="scroll-view_H" scroll-x="true" :show-scrollbar="false" @scroll="scroll">
				<span :class="{'menu-item':true,'selected': activeIndex === index}" v-for="(item,index) in 10"
					:key="index" @click="handlerMenu(index)">Java</span>
			</scroll-view>
		</view>
		<!-- 列表区域 -->
	...
	.category-box {
		...
		.menu {
			.scroll-view_H {
				...
				.selected {
					border-bottom: 1px solid #289fdf;
					color: #21b5ff;
				}
			}
